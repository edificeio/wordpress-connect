...
class OpenID_Connect_Generic_Client_Wrapper {
	...
	function alternate_redirect_uri_parse_request( $query ){
		if ($query->query_vars['category_name']=='openid-connect-authorize' || (isset( $query->query_vars['openid-connect-authorize'] ) &&
	...
	function authentication_request_callback() {
		...
		if( $this->settings->redirect_user_back && !empty( $redirect_url ) && strpos($redirect_url,"openid-connect-redirect")===false) {
